...
const Login = () => {
  ...
  const storeUser = localStorage.getItem('user') ? JSON.parse(localStorage.getItem('user')) : '';
  const [checkbox, setCheckBox] = useState(storeUser ? true : false);
  const [accountName, setAccountName] = useState(storeUser ? storeUser.accountName : '');
  const [password, setPassword] = useState(storeUser ? storeUser.password : '');
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-tr from-indigo-200 via-purple-200 to-pink-200">
      ...
      <form
        ...
        style={{ minWidth: 350 }}
      >
        <h2 className="text-3xl font-bold text-indigo-700 mb-4 text-center">Đăng nhập</h2>
        <div>
        ...
        <div className="flex justify-center items-center gap-1.5">
          <input type="checkbox" id="user" name="user" className="w-fit" checked={checkbox} onChange={(e)=>{
            if(e.target.checked) {
              setCheckBox(true);
            }
            else {
          ...
          {loading ? (
            <span className="flex items-center justify-center">
              <svg className="animate-spin h-5 w-5 mr-2 text-white" viewBox="0 0 24 24">
                ...
                <path className="opacity-70" fill="currentColor" d="M4 12a8 8 0 018-8v4l3-3-3-3v4a8 8 0 00-8 8z"/>
              </svg>
              Đang đăng nhập...
            </span>
          ) : (
        ...
        <div className={`text-center min-h-[24px] text-[15px] ${msg === "Correct bro" ? "text-green-600" : "text-red-500"}`}>
          {msg}
        </div>
